...
public class CountQuadCompression {
    ...
    /**
     * Step 1 - 모든 수 확인
     * 1. x, y 시작지점에서부터 arr size 까지 조회
     * 2. 매개변수로 전달된 2중 배열의 x, y 값을 바탕으로 2중 배열을 교차 검증하는 로직
     * 3. size 는 2중 배열의 크기, 즉 배열 행의 갯수, 4x4 = size 4
     * 4. 호출 시, size는 4로 설정되어 (0, 0)에서 시작하는 4x4 영역을 처리한다.
     * 모두 일치하면 true, 아니면 false 반환
     */
    private static boolean checkSameValue(int[][] grid, int x, int y, int size) {
        // 주어진 (x, y)의 값을 시작점으로 하여 size 범위 내의 모든 값을 조회
        // 초기 값은 (0,0)
        int target = grid[x][y];
    ...
    private static void dp(int x, int y, int size, int[][] grid) {
        // step 1. 현재 영역이 압축 가능하다면, result 배열에 값 추가하고 반환
